# Design Document: Randomized Tests for Subtle Decision Sabotage

## 1. Purpose

This document is the method contract for **Randomized Tests for Subtle Decision Sabotage**. It specifies what is
...
One-liner: Run small randomized checks for whether models can bias decisions without obvious red flags.

Hypothesis: Run small randomized checks for whether models can bias decisions without obvious red flags.
...
|---|---|---|
| Config schema | `decsab.configs.schema` | Typed dataclasses + role-keyed models |
| Loader | `decsab.configs.loader` | Hydra compose API |
| Device | `decsab.models.device` | MPS/CUDA/CPU resolution + fallback flag |
| Registry | `decsab.models.registry` | Pinned revisions + hardware fit |
| Hooks | `decsab.models.hooks` | Capture, steer, ablate, patch, attn intervene |
| Generation | `decsab.models.generation` | Chat template path recording |
| Data | `decsab.data` | Synthetic items, splits, manifests |
| Evaluation | `decsab.evaluation` | Metrics, failure harness, protocols |
| Ablation | `decsab.ablation` | Control / dropout / seed-sweep arms |
| Reporting | `decsab.reporting` | Aggregate → MD/TeX/figures |
| Cache | `decsab.cache` | Versioned artifact store |
...
`src/decsab/decsab/` are preserved across regeneration.
